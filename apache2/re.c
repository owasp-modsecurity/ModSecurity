...
msre_action *msre_create_action(msre_engine *engine, apr_pool_t *mp, const char *name, const char *param,
        char **error_msg)
{
    msre_action *action = apr_pcalloc(mp, sizeof(msre_action));
...
msre_actionset *msre_actionset_create(msre_engine *engine, apr_pool_t *mp, const char *text,
        char **error_msg)
{
    msre_actionset *actionset = (msre_actionset *)apr_pcalloc(mp,
    ...
    if (text != NULL) {
        if (msre_parse_actions(engine, mp, actionset, text, error_msg) < 0) {
            return NULL;
        }
    }
