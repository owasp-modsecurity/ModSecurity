...
void RuleWithActions::populate(const RuleWithActions *r) const {
    RuleWithActionsProperties::populate(r);
    if (m_actionMsg) {
        actions::ActionWithRunTimeString *arts = dynamic_cast<actions::ActionWithRunTimeString *>(m_actionMsg.get());
        if (arts != nullptr) {
            arts->populate(r);
        }
    }
    if (m_actionLogData) {
        actions::ActionWithRunTimeString *arts = dynamic_cast<actions::ActionWithRunTimeString *>(m_actionLogData.get());
        if (arts != nullptr) {
            arts->populate(r);
        }
    }
